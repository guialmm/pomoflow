  ...
  if (sessions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-500">
        ...
        <p className="text-sm mt-1">Start your first Pomodoro to see it here.</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-2 px-4 py-6">
      ...
      {sessions.map((s: Session, i: number) => (
        <div
          key={i}
          className="flex items-center justify-between bg-slate-800 rounded-xl px-4 py-3 gap-3"
        >
          <div className="flex flex-col min-w-0">
            <span className="text-slate-200 text-sm font-medium truncate">
              {s.task || <span className="text-slate-500 italic">no label</span>}
            </span>
